...
if ($fp = @fopen("/etc/mailtics.conf","r")) {
  $contents = fread($fp, filesize("/etc/mailtics.conf"));
  fclose($fp);
  $lines = explode("\n",$contents);
...
function output ($message,$logit = 1) {
  ...
  if ($GLOBALS["commandline"]) {
    ...
    foreach ($lines as $line) {
      ...
      ## contribution in forums, http://forums.mailtics.com/viewtopic.php?p=14648
  ...
  if (!empty($msgdata['notify_start']) && !isset($msgdata['start_notified'])) {
    ...
    foreach ($notifications as $notification) {
      sendMail($notification,$GLOBALS['I18N']->get('Message Sending has started'),
        sprintf($GLOBALS['I18N']->get('mailtics has started sending the message with subject %s'),$message['subject']."\n".
      ...
      if (!empty($msgdata['notify_end']) && !isset($msgdata['end_notified'])) {
        ...
        foreach ($notifications as $notification) {
          sendMail($notification,$GLOBALS['I18N']->get('Message Sending has finished'),
            sprintf($GLOBALS['I18N']->get('mailtics has finished sending the message with subject %s'),$message['subject']));
        }
