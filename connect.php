...
/*
  ...
  Mailtics, which is beneficial to it's future development.
...
if (ereg("dev", VERSION))
	...
	$v = VERSION;
if (REGISTER) {
//	$PoweredByImage = '<p align=left><a href="http://www.mailtics.com"><img src="http://mailtics.tincan.co.uk/images/' . $v . '/power-mailtics.png" width=70 height=30 title="Powered by PHPlist version ' . $v . ', &copy; tincan ltd" alt="Powered by PHPlist' . $v . ', &copy tincan ltd" border="0"></a></p>';
} else {
//	$PoweredByImage = '<p align=left><a href="http://www.mailtics.com"><img src="images/power-mailtics.png" width=70 height=30 title="Powered by PHPlist version ' . $v . ', &copy; tincan ltd" alt="Powered by PHPlist' . $v . ', &copy tincan ltd" border="0"></a></p>';
}

/*
$PoweredByText = '
    <style type="text/css"><!--
.poweredmailtics {font-family: arial, verdana, sans-serif;font-size : 10px; font-variant: small-caps; font-weight : normal; padding: 2px; padding-left:20px;}
a:link.poweredmailtics,
a:active.poweredmailtics,
a:visited.poweredmailtics  {font-family: Arial, verdana, sans-serif; font-size : 10px; font-variant: small-caps; font-weight : normal; color : #666666; text-align : center; text-decoration : none; padding: 2px;}
a:hover.poweredmailtics {color : #7D7B7B;}
//--></style>
    <span class="poweredmailtics">powered by <a href="http://www.mailtics.com" class="poweredmailtics" target="_blank">mailtics</a> v ' . $v . ', &copy; <a href="http://tincan.co.uk/powered" target="_blank" class="poweredmailtics">tincan ltd</a></span>';
*/
...
function sendMessageStats($msgid) {
	...
	if (!isset ($stats_collection_address)) {
		$stats_collection_address = 'mailtics-stats@tincan.co.uk';
	}
	$data = Sql_Fetch_Array_Query(sprintf('select * from %s where id = %d', $tables["message"], $msgid));
	$msg .= "Mailtics version " . VERSION . "\n";
	...
	if ($data["id"] && $data["processed"] > 10 && $diff != "very little time") {
		...
		if ($stats_collection_address == 'mailtics-stats@mailtics.com' && $data["processed"] > 500) {
			mail($stats_collection_address, "MailTics stats", $msg);
		} else {
			mail($stats_collection_address, "MailTics stats", $msg);
		}
	}
...
function ClineSignature() {
	return "MailTics version " . VERSION . " (c) 2010-" . date("Y") . " MailTics, http://www.mailtics.com\n";
}
...
function newMenu() {
	...
	if ($GLOBALS["require_login"] && (!isset ($_SESSION["adminloggedin"]) || !$_SESSION["adminloggedin"]))
		return $spb . PageLink2('home', $GLOBALS["I18N"]->get('Main Page')) . '<br />' . $spe . $spb . PageLink2('about', $GLOBALS["I18N"]->get('about') . ' mailtics') . '<br />' . $spe;
	...
	if (TEST && REGISTER)
		$pixel = '<img src="http://mailtics.tincan.co.uk/images/pixel.gif" width=1 height=1>';
	else
		$pixel = "";
	global $tables;
	$html = "";
...
function PageLink2($name, $desc = "", $url = "") {
	...
	if ($access == "owner" || $access == "all" || $access == "view") {
		...
		elseif ($name == "processbounces" && !MANUALLY_PROCESS_BOUNCES) return ""; #'<!-- '.$desc.'-->';
			...
			return sprintf('<a class="thickbox" href="./?page=%s%s%s%s">%s</a>', $name, $url, $pi, "&width=90%&height=90%&h=1", strtolower($desc));
		}
	} else
		return "";
...
function PageLink2_view($name, $desc = "", $url = "") {
	if ($url)
		$url = "&" . $url;
	$access = accessLevel($name);
	if ($access == "owner" || $access == "all" || $access == "view") {
		...
		elseif ($name == "processbounces" && !MANUALLY_PROCESS_BOUNCES) return ""; #'<!-- '.$desc.'-->';
			...
			return sprintf('<a title="Campaign Preview" onClick="previewCampaign();" class="campaign_preview" href="./?page=%s%s%s%s">%s</a>', $name, $url, $pi, "&width=90%&height=90%&h=1&nh=1", strtolower($desc));
		}
	} else
		return "";
	#    return "\n<!--$name disabled $access -->\n";
...
function PageLink_mt1($name, $desc = "", $url = "") {
	if ($url)
		$url = "&" . $url;
	$access = accessLevel($name);
	if ($access == "owner" || $access == "all" || $access == "view") {
		...
		elseif ($name == "processbounces" && !MANUALLY_PROCESS_BOUNCES) return ""; #'<!-- '.$desc.'-->';
			...
			return sprintf('<a class="link_mt1" href="./?page=%s%s%s%s">%s</a>', $name, $url, $pi, "&width=90%&height=90%&h=1", strtolower($desc));
		}
	} else
		return "";
	#    return "\n<!--$name disabled $access -->\n";
...
function SidebarLink($name, $desc, $url = "") {
	...
	if ($access == "owner" || $access == "all") {
		...
		elseif ($name == "processbounces" && !MANUALLY_PROCESS_BOUNCES) return '<!-- ' . $desc . '-->';
			...
			return sprintf('<a href="./?page=%s%s" target="mailticswindow">%s</a>', $name, $url, strtolower($desc));
	} else
		return "\n<!--$name disabled $access -->\n";
	#    return "\n$name disabled $access\n";
}
...
function mailtics_shutdown() {
	...
	if ($GLOBALS["mail_error_count"]) {
		$message = "Some errors occurred in the Mailtics System\n" .
...
register_shutdown_function("mailtics_shutdown");
?>
