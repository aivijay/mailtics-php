...
if (PHPMAILER && is_file(dirname(__FILE__).'/phpmailer/class.phpmailer.php')) {
  # mailticsmailer, extended of the popular phpmail class
...
#0011857: forward to friend, retain attributes
  ...
  $text["signature"] = "\n\n--\nPowered by Mailtics, www.mailtics.com --\n\n";
...
*/
  if (!EMAILTEXTCREDITS) {
    $html["signature"] = $PoweredByImage;#'<div align="center" id="signature"><a href="http://www.mailtics.com"><img src="powermailtics.png" width=88 height=31 title="Powered by Mailtics" alt="Powered by PHPlist" border="0"></a></div>';
    # oops, accidentally became spyware, never intended that, so take it out again :-)
    $html["signature"] = preg_replace('/src=".*power-mailtics.png"/','src="powermailtics.png"',$html["signature"]);
  } else {
...
#    preg_match_all('/<a href=([^> ]*)([^>]*)>(.*)<\/a>/Umis',$htmlmessage,$links);
    ...
    for($i=0; $i<count($links[2]); $i++){
      ...
      $linkid = 0;
#      print "LINK: $link<br/>";
      if ((preg_match('/^http|ftp/',$link) || preg_match('/^http|ftp/',$urlbase)) && $link != 'http://www.mailtics.com' && !strpos($link,$clicktrack_root)) {
...
#      $link = strtolower(cleanUrl($links[1][$i]));
      ...
      $linkid = 0;
      if (preg_match('/^http|ftp/',$link) && $link != 'http://www.mailtics.com' && !strpos($link,$clicktrack_root)) {
        $url = cleanUrl($link,array('PHPSESSID','uid'));
        $req = Sql_Query(sprintf('insert ignore into %s (messageid,userid,url,forward)
...
#  }
  ...
  if (!PHPMAILER) {
    $mail = new html_mime_mail(
      array('X-Mailer: Mailtics v'.VERSION,
...
function createPDF($text) {
  ...
  $pdf=new FPDF();
  $pdf->SetCreator("Mailtics version ".VERSION);
  $pdf->Open();
  $pdf->AliasNbPages();
  $pdf->AddPage();
...
Sql_Query(sprintf('select * from %s where filename = "%s" and template = 0',
  $GLOBALS["tables"]["templateimage"],"powermailtics.png"));
if (!Sql_Affected_Rows())
  Sql_Query(sprintf('insert into %s (template,mimetype,filename,data,width,height)
  values(0,"%s","%s","%s",%d,%d)',
  $GLOBALS["tables"]["templateimage"],"image/png","powermailtics.png",
  $newpoweredimage,
  70,30));
