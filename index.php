...
if (!$GLOBALS["commandline"] && isset($GLOBALS["developer_email"]) && $_SERVER['HTTP_HOST'] != 'cvs.mailtics.com' && $GLOBALS['show_dev_errors']) {
...
if ($GLOBALS["commandline"]) {
  ...
  if (is_array($GLOBALS["commandline_users"]) && sizeof($GLOBALS["commandline_users"]) && !in_array($_SERVER["USER"],$GLOBALS["commandline_users"])) {
    clineError("Sorry, You (".$_SERVER["USER"].") do not have sufficient permissions to run mailtics on commandline");
    exit;
  }
...
if (isset($GLOBALS["require_login"]) && $GLOBALS["require_login"]) {
  if ($GLOBALS["admin_auth_module"] && is_file("auth/".$GLOBALS["admin_auth_module"])) {
    ...
    if ($GLOBALS["admin_auth_module"]) {
      logEvent("Warning: unable to use ".$GLOBALS["admin_auth_module"]. " for admin authentication, reverting back to mailtics authentication");
      $GLOBALS["admin_auth_module"] = 'mailtics_auth.inc';
    }
    require_once 'auth/mailtics_auth.inc';
  }
...
if (checkAccess($page,"") || $page == 'about') {
  ...
  //// vijay REMOVE    include "lan/".$_SESSION['adminlanguage']['iso']."/".$include;
    }
  #  print "Including $include<br/>";
    ...
    if (!$parses_ok) {
      ...
      print '<p>Sorry, an error occurred. This is a bug. Please <a href="http://www.mailtics">report the bug to the Bug Tracker</a><br/>Sorry for the inconvenience</a></p>';
    } else {
