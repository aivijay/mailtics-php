...
# sidebar page
  ...
  $html.= '<title>PHPlist Mozilla Menu</title>
    <link href="styles/mailtics_main.css" type=text/css rel=stylesheet></head>
    <body bgcolor="#ffffff">';
...
#      $html .= $spb.sprintf('<a href="%s&id=%d" target="mailticswindow">%s</a>',getConfig("subscribeurl"),$row["id"],$row["title"]).$spe;
#     }
#  } else {
#    $html .= $spb.sprintf('<a href="%s" target="mailticswindow">%s</a>',getConfig("subscribeurl"),$GLOBALS["strSubscribeTitle"]).$spe;
#  }
#  $url = getConfig("unsubscribeurl");
#  if ($url)
#    $html .= $spb.'<a href="'.$url.'" target="mailticswindow">Unsubscribe</a>'.$spe;
#  else
#    $html .= $spb.'<a href="../?p=unsubscribe" target="mailticswindow">Sign Off</a>'.$spe;
