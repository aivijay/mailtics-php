...
print '<script type="text/javascript" src="/js/ui/ui.core.js"></script>';
print '<script type="text/javascript" src="/js/ui/ui.progressbar.js"></script>';

print '<script type="text/javascript">function pupdate(cvalue,ctotal) {
document.getElementById("p_count").disabled = "true";
document.getElementById("p_count").value = cvalue + " of " + ctotal;
      }
function pupdateFinish() {
document.getElementById("disptag").value = "Upload is completed.";
}
</script>';
  ...
  foreach ($_SESSION["import_attribute"] as $column => $rec) {
    ...
    if (trim($column) != '') {
      ...
      elseif ($rec["record"] == "skip") {
        ...
        $ls->addColumn($column, $GLOBALS['I18N']->get('maps to'), $system_attributes[$rec["record"]]);
      }
...
if (sizeof($email_list)) {
  ...
  if (sizeof($email_list) > 300 && !$_SESSION["test_import"]) {
    ...
    # show the email id upload count
print '<div style="padding:10px 10px 10px 10px; margin:5px 5px 15px 15px; width:320px; height:60px; border:1px solid #998889;">
      <center><input type="text" id="p_count" style="align:centre; color:red;"/><br><br>
      <input type="text" value=" Uploading email ids in progress... " id="disptag" style="border:0px;width:250px;"></center>
      </div>';
  ...
  foreach ($email_list as $line) {
	...
	# dynamicall update the email ids total and update count for progress bar
	print '<script type="text/javascript">pupdate('.$cnt.','.$total.');</script>';		
  ...
  if (!$_SESSION["test_import"]) {
    ...
    print '<script language="Javascript" type="text/javascript"> pupdateFinish(); </script>';
    ...
    if (!$GLOBALS['commandline']) {
      print $report;
      if (function_exists('sendmail')) {
        sendMail(getConfig("admin_address"), $GLOBALS['I18N']->get('mailtics Import Results'), $report);
      }
...
printf($GLOBALS['I18N']->get('mailticsuploadlimit'),IMPORT_FILESIZE);?>
</td></tr>
