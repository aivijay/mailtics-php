...
function getAttributeIDbyName ($sName) {
  ...
  # workaround for integration webbler/mailtics
...
#   sendError("Attribute without Tablename $attid");
  ...
  switch ($data["type"]) {
    ...
    case "password":
      ...
      break;
    case "select":
      $curval = Sql_Fetch_Row_Query(sprintf('select id from mailtics_listattr_%s
        where name = "%s"',$atttable,$data["displayvalue"]),1);
      if (!$curval[0] && $data['displayvalue'] && $data['displayvalue'] != '') {
        Sql_Query(sprintf('insert into mailtics_listattr_%s (name) values("%s")',$atttable,
          $data["displayvalue"]));
...
/*  if (is_array($GLOBALS["config"]["bocs_dpa"])) {
  ...
  if (!$res && is_object($GLOBALS["config"]["plugins"]["mailtics"])) {
    $mailtics = $GLOBALS["config"]["plugins"]["mailtics"];
    foreach ($_SESSION["userdata"] as $key => $field) {
      if (($field["formtype"] == "List Subscription" || $field["type"] == "List Subscription") && $field["listid"]) {
         ...
         if ($field["value"] && isset($_SESSION["userdata"]["email"])) {
           if ($mailtics->addEmailToList($_SESSION["userdata"]["email"]["value"],$listid)) {
             $mailtics->confirmEmail($_SESSION["userdata"]["email"]["value"]);
             #  sendError("User added to list: $listid");
           } else {
             # sendError("Error adding user to list: $listid");
           }
         } #else {
           #$mailtics->removeEmailFromList($_SESSION["userdata"]["email"]["value"],$listid);
         #}
       }
    }
  }
