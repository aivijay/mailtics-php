    ...
    if (!$error || $force) {
      if ($table == "admin") {
        ...
        Sql_Query(sprintf('insert into %s values(0,"%s","%s","%s",now(),now(),"%s","%s",now(),%d,0)',
          $tables["admin"],"admin","admin","",$adminname,"mailtics",1));
      } elseif ($table == "task") {
...
if ($success) {
  ...
  printf('<p>'.$GLOBALS['I18N']->get('Success').': <a href="mailto:mailtics2@tincan.co.uk?subject=Successful installation of mailtics&body=%s">'.$GLOBALS['I18N']->get('Tell us about it').'</a>. </p>', $body);
  printf('<p>
  ...
  print "<p>".$GLOBALS['I18N']->get("Continue with")." ".PageLink2("setup",$GLOBALS['I18N']->get("Mailtics Setup"))."</p>";
} else {
