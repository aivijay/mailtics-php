...
if (Sql_Table_exists($tables["config"],1)) {
  ...
  if ($dbversion != VERSION && !defined("IN_WEBBLER")) {
    Error($GLOBALS['I18N']->get('Your database is out of date, please make sure to upgrade').'<br/>'.
     ...
     $GLOBALS['I18N']->get('mailtics version').' : '.VERSION
     );
    $upgrade_required = 1;
  }
} else {
  ...
  if ($needscheck[0]) {
    ...
    if ($fp = @fopen ("http://www.mailtics.com/files/LATESTVERSION","r")) {
      ...
      if (!versionCompare($thisversion,$latestversion)) {
        ...
        print '<a href="http://mantis.mailtics.com/changelog_page.php">'.$GLOBALS['I18N']->get('View what has changed').'</a>&nbsp;&nbsp;';
        print '<a href="http://www.mailtics.com/download">'.$GLOBALS['I18N']->get('Download').'</a></div>';
      }
    }
...
if ($some) {
  $tabStringStart = '<div id="main"><div id="tabs-1">';
  $tabStringEnd = '</div></div>'; 
  print $tabStringStart . $ls->display() . $tabStringEnd;
}
