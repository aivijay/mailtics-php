...
<script language="Javascript" src="js/view_campaign.js" type="text/javascript"></script>
<hr>
...
$tabs->addTab($GLOBALS['I18N']->get("draft"),PageUrl2("campaigns&type=draft"));
...
// fix vijay for all campaigns to be listed
$subselect = ' status in ("submitted", "suspended", "prepared", "draft", "sent", "inprocess") ';
$url_keep = '&type=all';
...
if (isset($start) && $start > 0) {
  $listing = $GLOBALS['I18N']->get("Listing campaigns")." $start ".$GLOBALS['I18N']->get("to")." " . $end;
  $limit = "limit $start,".MAX_MSG_PP;
} else {
  $listing =  $GLOBALS['I18N']->get("Listing campaigns 1 to")." ".$end;
  $limit = "limit 0,".MAX_MSG_PP;
  $start = 0;
}
print "<span class='title1'>" . $total. " ".$GLOBALS['I18N']->get("Campaigns")."</span></p>";
...
if ($total)
  printf ('<table border=0><tr><td colspan=4 align=center>%s</td><td>%s</td><td>%s</td><td>
          %s</td><td>%s</td></tr></table>',
          $listing,
          PageLink2("campaigns$url_keep","&lt;&lt;","start=0"),
          PageLink2("campaigns$url_keep","&lt;",sprintf('start=%d',max(0,$start-MAX_MSG_PP))),
          PageLink2("campaigns$url_keep","&gt;",sprintf('start=%d',min($total,$start+MAX_MSG_PP))),
          PageLink2("campaigns$url_keep","&gt;&gt;",sprintf('start=%d',$total-MAX_MSG_PP)));
...
//  print "<td>".$GLOBALS['I18N']->get("Message info")."</td><td>".$GLOBALS['I18N']->get("Status")."</td><td>".$GLOBALS['I18N']->get("Action")."</td></tr>";
  $result = Sql_query("SELECT *, date_format(entered,'%b %d %Y %r') as entered_df, date_format(embargo,'%b %d %Y %r') as embargo_df  FROM ".$tables["message"]." $subselect order by status,entered desc $limit");
  $sl_no = $start+1;
  while ($msg = Sql_fetch_array($result)) {
    ...
    printf ('<tr class="' . $row_class . '">
          <td valign="top">%s</td> 
          <td valign="top">%s</td>
          <td valign="top">%s</td>
          <td valign="top">%s</td>',
      $sl_no,
      PageLink_mt1("campaigns", stripslashes($msg["campaign"]) ,"id=".$msg["id"]),
      $msg["entered_df"],
      $msg["embargo_df"]
    );
/*
...
*/
    ...
    if ($msg['status'] == 'sent') {
      ...
      $sendstats =
        sprintf('<br /><table border=0 cellpadding=2 cellspacing=0>
        <tr>
          <td style="font-weight: bold; color: black;">'.$GLOBALS['I18N']->get("total").'</td>
          <td style="font-weight: bold; color: black;">'.$GLOBALS['I18N']->get("text").'</td>
          <td style="font-weight: bold; color: black;">'.$GLOBALS['I18N']->get("html").'</td>
          <td style="font-weight: bold; color: black;">'.$GLOBALS['I18N']->get("PDF").'</td>
          <td style="font-weight: bold; color: black;">'.$GLOBALS['I18N']->get("both").'</td>
        </tr>
        <tr>
    ...
    printf('
      ...
      %s &nbsp;&nbsp;
      %s
      <a href="javascript:deleteRec(\'%s\');">'. "<img src='images/edit-delete-icon16.png' border='0' alt='Delete Campaign'/>" .'</a>
      </td>
      </tr>',
      $status.
      $sendstats,
      PageLink2_view("preview_campaign","<img src='images/view-text-icon16.png' border='0'/>", "id=".$msg["id"]),
      $msg["status"] != 'prepared' ? PageLink2("send","<img src='images/edit-icon16.png' border='0'/>","id=".$msg["id"]) : PageLink2("preparesend",$GLOBALS['I18N']->get("Edit"),"id=".$msg["id"]),
      $msg['status'] != 'inprocess' ? PageLink2("campaigns","<img src='images/reload-icon16.png' border='0'/>","resend=".$msg["id"]) : $totalsent." ".$GLOBALS['I18N']->get("sent"),
      $clicks[0] && CLICKTRACK ? PageLink2("mclicks",$GLOBALS['I18N']->get("click stats"),"id=".$msg["id"]).'<br/>':'',
      PageURL2("campaigns$url_keep","","delete=".$msg["id"])
    );
    $sl_no++;
  }
}
