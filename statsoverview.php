...
if (!$id) {
  ...
  <div class="b_header">
  <h5>Campaign based statistics view, click on the campaign id to view complete details.</h5>
  ...
  $req = Sql_Query(sprintf('select msg.owner,msg.campaign as campaignname,msg.id as messageid,count(um.viewed) as views, count(um.status) as total,subject,date_format(sent,"%%e %%b %%Y") as sent,bouncecount as bounced from %s um,%s msg where um.messageid = msg.id %s %s
  ...
  $ls = new WebblerListing($GLOBALS['I18N']->get(' Last few messages <br>'));
   
  while ($row = Sql_Fetch_Array($req)) {  	
    
  	//$element = $row['messageid'].' '.substr($row['subject'],0,50);
  	// Need to have campaign id and the name other wise it will be grouped based on the campaign names
    	
	$element = "Campaign Id ".$row['messageid'];
  	
  	//$element = $row['messageid'].'  '.$row['campaignname'];
    $ls->addElement($element,PageURL2('message&id='.$row['messageid']));
    
    $ls->addColumn($element,'campaign',$row['campaignname']);    
