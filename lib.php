...
if (!defined("NAME")) define("NAME",'MailTics');
...
if (!isset($GLOBALS["admin_auth_module"])) $GLOBALS["admin_auth_module"] = 'mailtics_auth.inc';
if (!isset($GLOBALS["require_login"])) $GLOBALS["require_login"] = 0;

# default values for JQRT editor
if (!defined('USEJQRT')) define('USEJQRT',0);
if (!defined('JQRTUPLOAD_DIR')) define('JQRTUPLOAD_DIR','uploadfiles');
...
if (defined("IN_WEBBLER") && is_object($GLOBALS["config"]["plugins"]["mailtics"])) {
  $GLOBALS["tables"] = $GLOBALS["config"]["plugins"]["mailtics"]->tables;
  $GLOBALS["table_prefix"] = $GLOBALS["config"]["plugins"]["mailtics"]->table_prefix;
}

$usephpmailer = 0;
if (PHPMAILER && is_file(dirname(__FILE__).'/phpmailer/class.phpmailer.php')) {
  include_once dirname(__FILE__) . '/class.mailticsmailer.php';
  $usephpmailer = 1;
}
...
ini_set('error_append_string','<font style=\"{font-variant: small-caps;font-size: 12px}\">mailtics</font> version '.VERSION);
ini_set('error_prepend_string','<P><font color=red style=\"{font-size: 12px}\">Sorry a software error occurred:</font><br/>
  Please <a href="http://mantis.mailtics.com">report a bug</a> when reporting the bug, please include URL and the entire content of this page.<br/>');
...
function system_messageHeaders($useremail = "") {
  ...
  $additional_headers .= "X-Mailer: mailtics version $v (www.mailtics.com)\n";
...
#    logEvent($url.' is not cached in database '.$timeout.' '. $dbcache_lastmodified." ".time());
  ...
  $headreq->addHeader('User-Agent', 'mailtics v'.VERSION.' (http://www.mailtics.com)');
  if (!PEAR::isError($headreq->sendRequest(false))) {
    ...
    ## see http://mantis.mailtics.com/view.php?id=7684
#    $lastmodified = strtotime($header["last-modified"]);
    ...
    if (!$cache) {
      ...
      $req->addHeader('User-Agent', 'mailtics v'.VERSION.' (http://www.mailtics.com)');
      logEvent('Fetching '.$url);
