...
{
	// Initializes the MailTics object.
	var oMailTics = new Object() ;
	oMailTics.Address = '' ;
	oMailTics.Subject = '' ;
	oMailTics.Body = '' ;
	...
	if ( aLinkInfo && aLinkInfo[1] == 'mailto' )
	{
		...
		if ( aParts )
		{
			...
			oMailTics.Address = aParts[1] ;

			// Look for the optional e-mail parameters.
			if ( aParts[2] )
			{
				var oEMailParams = oParser.ParseEMailParams( aParts[2] ) ;
				oMailTics.Subject = oEMailParams.Subject ;
				oMailTics.Body = oEMailParams.Body ;
			}
		}
		return oMailTics ;
	}
	else if ( aLinkInfo && aLinkInfo[1] == 'javascript' )
	{
		...
		if ( func != null )
		{
			try
			{
				...
				if ( aMatch )
				{
					...
					// Fill the MailTics object that will be returned
					oMailTics.Address = aInfo['NAME'] + '@' + aInfo['DOMAIN'] ;
					oMailTics.Subject = decodeURIComponent( aInfo['SUBJECT'] ) ;
					oMailTics.Body = decodeURIComponent( aInfo['BODY'] ) ;
		...
		{
			// The link is encoded
			oMailTics.Address = eval( aMatch[1] ) ;
			if ( aMatch[2] )
			{
...
function LoadSelection()
{
	...
	var oMailTics = oParser.ParseEMailUri( sHRef );

	if ( oMailTics )
	{
		sType = 'email' ;

		GetE('txtEMailAddress').value = oMailTics.Address ;
		GetE('txtEMailSubject').value = oMailTics.Subject ;
		GetE('txtEMailBody').value    = oMailTics.Body ;
	}
	else if ( sProtocol )
	{
